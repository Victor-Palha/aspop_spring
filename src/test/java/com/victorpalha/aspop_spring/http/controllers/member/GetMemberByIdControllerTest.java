...
@Testcontainers
public class GetMemberByIdControllerTest {
    @Container
    ...
    @Test
    public void should_get_member_by_id() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.get("/api/member/".concat(memberMock.getMemberId()))
                        .contentType(MediaType.APPLICATION_JSON)
                        .header("Authorization", "Bearer " + jwtToken)
                )
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.message").value("Membro encontrado"))
                .andExpect(jsonPath("$.data.memberId").value(mockMember.getMemberId()));
    }

    @Test
    public void should_not_get_member_by_id_if_doesnt_exists() throws Exception {
        String jwtToken = jwtProvider.generateMainToken(
                "123123",
                MemberRole.ADMIN,
                Instant.now().plusSeconds(60)
        );
        mockMvc.perform(MockMvcRequestBuilders.get("/api/member/".concat("banana"))
                        .contentType(MediaType.APPLICATION_JSON)
                        .header("Authorization", "Bearer " + jwtToken)
                )
